...
그룹 제한:
- 그룹 최대 인원은 **10명**입니다. 가득 찬 그룹에는 초대가 불가합니다.
...
**User 레벨의 -1 탐색 제한:**
- User / Server 레벨은 -1 depth로 전체 탐색을 **24시간에 1회** 사용할 수 있습니다.
- 쿨다운 중에는 depth -1을 입력해도 자동으로 3으로 제한됩니다.
- SCAN 화면에서 -1 입력 시 쿨다운 남은 시간 또는 사용 가능 여부가 표시됩니다.
- Creator / Builder / Admin 레벨은 제한 없이 -1을 사용할 수 있습니다.
...
## 공지 전송 제한 (Notice Cooldown)

공지(공지S, 공지L)는 메시 네트워크 전체에 전파되는 high-cost 메시지입니다.

**현재 상태:** 테스트 및 실험 목적으로 쿨다운 제한이 **해제**되어 있습니다. 모든 레벨에서 횟수 제한 없이 공지를 보낼 수 있습니다.

**향후 적용 예정 쿨다운 정책:**

| 레벨 | 공지S (단거리) | 공지L (전체 mesh) |
|---|---|---|
| Creator | 제한 없음 | 제한 없음 |
| Builder | 제한 없음 | 제한 없음 |
| Admin | 24시간 N회 (TBD) | 24시간 N회 (TBD) |
| User | 24시간 N회 (TBD) | 24시간 N회 (TBD) |
| Server | 전송 불가 | 전송 불가 |

쿨다운 정책은 네트워크 규모와 실제 사용 패턴을 보고 확정할 예정입니다.
