...
| **Heartbeat** | 30초 PING/PONG, 2회 무응답 시 자동 이웃 제거 |
| **TOFU 신뢰** | QR 핑거프린트 대조로 공개키 진위 확인 |
| **오프라인 백업** | 신원(키쌍) 내보내기/복원 지원 |
...
├── cryptography         Ed25519 서명 + X25519 ECDH + AES-GCM
...
│   ├── lib/
│   │   ├── core/
│   │   │   ├── ble/              BLE 서비스 (스캔·연결·GATT·fragment)
│   │   │   ├── crypto/           암호화 서비스 (Ed25519·X25519·AES-GCM)
│   │   │   ├── packet/           패킷 구조 (직렬화·역직렬화·서명)
│   │   │   └── storage/          SQLite DB (v8 마이그레이션)
│   │   ├── features/
│   │   │   ├── identity/         기기 ID·키쌍·역할·백업·복원
│   │   │   ├── contacts/         연락처·TOFU·즐겨찾기·그룹
│   │   │   ├── messaging/        메시지 송수신·릴레이·정책
│   │   │   └── settings/         앱 설정
│   │   └── ui/
│   │       ├── home/             연락처 목록·그룹·필터
│   │       ├── chat/             1:1 채팅 (일반·타임드·공지)
│   │       └── qr/               QR 생성·스캔·TOFU 확인
...
| 역할 | 설명 | 공지 권한 |
|------|------|-----------|
| Creator | 네트워크 창설자 | 무제한 |
| Builder | 핵심 운영자 | 공지S 1시간, 공지L 2시간 |
| Admin | 관리자 | 공지S 2시간, 공지L 4시간 |
| User | 일반 사용자 | 공지S 6시간, 공지L 24시간 |
| Server | 자동화 노드 | 공지 불가 |
...
| Wave 3 | 🔄 진행 | 실기기 검증·버그 수정 (2홉 릴레이 확인) |
| Phase 1 | 🔄 진행 | UI 완성 (앱 셸·연락처·채팅·공지) |
| Phase 2 | ⏳ 예정 | 그룹 메시지·N-depth 토폴로지 시각화 |
| Phase 3 | ⏳ 예정 | 관리자 공지·기여도 측정 |
