...
class ContactFileService {
  ...
  Future<int> importFromJson(String rawJson) async {
    ...
    if (decoded is! Map<String, dynamic>) {
      throw const FormatException('Invalid contacts file.');
    }

    final contacts = decoded['contacts'];
    if (contacts is! List) {
      throw const FormatException('Contacts file has no contacts list.');
    }
    ...
    for (final item in contacts) {
      ...
      final encryptionPublicKey = _hexToBytes(
        item['encryptionPublicKey'] as String?,
      );
